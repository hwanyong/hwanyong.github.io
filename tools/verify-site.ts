...
/*
 ...
 ★ 이 저장소는 취소선을 편집 장치로 쓰지 않는다. "이건 틀렸다" 를 보이려면 글을 고치고
   ★개정본을 올린다★ — 개정축이 이미 그 일을 하며(docs/lecture.md), 그쪽은 무엇이
   언제 바뀌었는지 독자가 비교할 수 있다. 본문에 그어 놓은 선은 그 정보를 주지 않고
   읽는 사람을 헷갈리게만 한다.

 ★ 이 검사가 있는 진짜 이유는 규율이 아니라 ★사고★ 다. 한국어에서 범위를 적는 관용은
   `7~10` 인데 GFM 은 `~` 를 취소선 구획 문자로 읽는다. 한 문단에 범위가 두 번 나오면
   두 물결표가 짝을 이뤄 그 사이가 통째로 그어진다 — 실제로 발행된 글에서 두 군데가
   그렇게 나갔고, 소스만 봐서는 아무 이상이 없어 눈으로는 잡히지 않았다.
   그래서 소스가 아니라 ★산출물★ 을 본다. 범위는 en dash(–) 로 적을 것.
*/
for (const file of pages) {
  if (!readFileSync(file, 'utf8').includes('<del>')) continue;
  fail.push(
    `취소선이 발행되었습니다: ${file}\n` +
      `      의도한 것이라면 취소선 대신 글을 고치고 개정본을 올리세요.\n` +
      `      의도하지 않았다면 원인은 거의 언제나 범위 표기의 물결표입니다(7~10 → 7–10).`,
  );
}
