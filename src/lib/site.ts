...
export const ADSENSE = {
  /**
   * 캐나다 계정. 콘솔의 계정 정보와 대조해 확인 (2026-08-21).
   *
   * 구 한국 계정 pub-5888836555193733 에서 옮겨왔다. AdSense 는 결제 주소의
   * 국가를 변경할 수 없어(answer/2628816) 계정을 새로 만드는 것 외에 방법이 없다.
   * 이 값을 바꿀 때 public/ads.txt 도 반드시 함께 고칠 것 —
   * 둘이 어긋나면 크롤러가 게시자를 인증하지 못해 광고가 통째로 미노출된다.
   */
  client: 'ca-pub-3893186228911656',

  /**
   * 로더(adsbygoogle.js) 송출 여부.
   *
   * ★ 로더와 슬롯의 축을 나눈 이유:
   *   사이트 심사를 받으려면 로더가 페이지에 실제로 실려 있어야 하는데,
   *   그 시점에는 아직 광고 단위가 없어 유효한 data-ad-slot 값이 존재하지 않는다.
   *   하나의 플래그로 묶으면 "심사는 받아야 하는데 켜면 자리표시자 슬롯이 나간다"는
   *   교착이 생긴다. 두 소비처가 각각 자기 축을 읽는다
   *   (AdSenseHead=loader, AdUnit=units).
   *
   *   loader=true,  units=false  ← 심사 대기. 지금 여기.
   *   loader=true,  units=true   ← 광고 단위 발급 후
   *   loader=false               ← 광고를 통째로 내림. units 값은 무의미해진다.
   */
  loader: true,

  /**
   * 수동 광고 단위(<ins class="adsbygoogle">) 송출 여부.
   *
   * false 인 이유: AD_SLOTS 가 아직 자리표시자다.
   * 없는 슬롯으로 요청하면 지면이 빈 채로 남고 콘솔에 오류만 쌓인다.
   *
   * 켜는 순서:
   *   1. 사이트 심사 통과
 ...
 * ADSENSE.units 가 false 인 동안에는 렌더되지 않으므로 자리표시자여도 무해하다.
 */
